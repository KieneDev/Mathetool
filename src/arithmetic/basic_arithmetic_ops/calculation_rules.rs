use crate::paths::calc_nums::{calculate_numbers_addition, calculate_numbers_mult_diff};
...
pub fn rules_for_calculation(formula: Vec<String>){
    ...
    if operator_count[0] > 0 {
        ...
        if let Some(temp) = operator_count.get_mut(0) {*temp = 0;}
    }
...
fn count_operators(numbers: &Vec<String>) -> Vec<usize> {
    ...
    println!("Vektorinhalt: {:?}", operator_count);
